...
public abstract class ShopCreationData {
	...
	protected ShopCreationData(	Player creator, ShopType<?> shopType, ShopObjectType<?> shopObjectType,
		...
		if (creator != null) {
			...
			this.creatorUser = ShopkeepersAPI.getUserManager().getUser(creator);
			assert creatorUser != null; // since player is online
			// Explicit check, just in case:
			Validate.State.notNull(creatorUser, "Missing user for the creating player!");
		} else {
