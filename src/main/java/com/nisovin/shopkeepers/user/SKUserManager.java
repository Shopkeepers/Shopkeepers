...
public class SKUserManager implements UserManager {
	...
	// Mappings by UUID string and (normalized) name: Allow for fast lookups and completions of partial uuid/name inputs
	// (eg. for use in commands).
	...
	// user object has already been GC'ed.
	...
	private void addNameMapping(SKUser user) {
		assert user != null;
		String normalizedName = this.getNormalizedName(user); // can be null
	...
	private void updateNameMapping(SKUser user) {
		...
		String normalizedNewName = normalizeUserName(user.getName()); // can be null
	...
	// Can return null
	private String getNormalizedName(SKUser user) {
		assert user != null;
		return normalizeUserName(user.getName());
	}
	...
	// should do so within the AsyncPlayerPreLoginEvent to not block the main server thread then.
	...
	@Override
	public Stream<SKUser> getUsers(String playerName, boolean checkDisplayNames) {
		return null; // TODO
	}

	@Override
	public Stream<SKUser> getUsersByUUIDPrefix(String uuidPrefix) {
		...
		return Utils.mapReferents(prefixMap.values().stream());
	}

	@Override
	public Stream<SKUser> getUsersByNamePrefix(String namePrefix, boolean checkDisplayNames) {
		Validate.notNull(namePrefix, "namePrefix is null");
		String normalizedNamePrefix = this.normalizeUserName(namePrefix);
		SortedMap<String, List<WeakReference<SKUser>>> prefixMap = byName.tailMap(normalizedNamePrefix);
		return prefixMap.values().stream().flatMap(refList -> Utils.mapReferents(refList.stream()));
	}
}
