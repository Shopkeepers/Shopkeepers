...
* `Inventory#equals` returns `true` for two inventory instances that are backed by the same underlying Minecraft inventory, e.g. the inventories retrieved via two separate `Container#getInventory()` calls for the same container block.
  * For instance, CraftBukkit creates a new Bukkit inventory instance on each call, but compares the underlying Minecraft inventories in its `#equals` implementation.
  * This might not apply to double chest inventories: In CraftBukkit, each lookup creates a new underlying Minecraft inventory that combines the inventories of both chests. To check if two double chest inventories refer to the same double chest, we therefore instead compare the inventories of their sides (`DoubleChestInventory#getLeftSide()` and `#getRightSide()`).
